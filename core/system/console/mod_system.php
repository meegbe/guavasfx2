...
class mod_system
{
    ...
    protected function licenseMethod()
    {
        //
    }
    protected function keyMethod()
    {
        //
    }
    protected function updateMethod()
    {
        $pg = new Bar(100, "Downloading: [:bar] - :current/:total - :percent% - Elapsed::elapseds - ETA::etas - Rate::rate/s");
        for ($i = 1; $i <= 100; $i++) {
            usleep(50000);
            $pg->tick();
        }
        //$pg->update(100);
        $pg = new Bar(100, "Installing: [:bar] - :current/:total - :percent% - Elapsed::elapseds - ETA::etas - Rate::rate/s");
        for ($i = 1; $i <= 100; $i++) {
            usleep(50000);
            $pg->tick();
        }
...
class Bar
{
    ...
    public function __construct($total = 1, $format = "Progress: [:bar] - :current/:total - :percent% - Elapsed::elapseds - ETA::etas - Rate::rate/s", $stream = STDERR)
    {
        ...
        if(strcasecmp(substr(PHP_OS, 0, 3), 'WIN') == 0){
            ...
            {
                if(preg_match('~(Columns|columns):([\\w.\ ]+)~', $ar, $matched))
                {
                    $this->width = (int)trim($matched[2]);
                }
            } 
        } else {
    ...
    private function drawBar()
    {
        ...
        if(!$this->reDraw) {
            ...
            if (strpos($output, ':bar') !== false) {
                ...
                $output = str_replace(':bar', str_repeat($this->symbolComplete, (int)max($done, 0)) . str_repeat($this->symbolIncomplete, (int)max($left, 0)), $output);
            }
